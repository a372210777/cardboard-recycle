...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    {
        byte uuidBytes[] = UUID.randomUUID().toString().getBytes(Charset.forName("UTF-8"));
        byte randomBytes[] = gnerateSaltByte(byteSize);
        byte finalBytes[] = Arrays.copyOf(uuidBytes, uuidBytes.length + randomBytes.length);
        System.arraycopy(randomBytes, 0, finalBytes, uuidBytes.length, randomBytes.length);
        String encodeResult = Base64.encodeBase64String(finalBytes);
        encodeResult = StringUtils.replace(encodeResult, "/", "_");
        encodeResult = StringUtils.replace(encodeResult, "+", "-");
        encodeResult = StringUtils.replace(encodeResult, "=", "");
        return encodeResult;
    }
